...
const FEATURES = [
  {
    ...
    accent: 'from-emerald-500/10 to-teal-500/5 border-emerald-300',
  ...
  },
  {
    ...
    accent: 'from-indigo-500/10 to-violet-500/5 border-indigo-300',
  ...
  },
  {
    ...
    accent: 'from-sky-500/10 to-cyan-500/5 border-sky-300',
  ...
  },
  {
    ...
    accent: 'from-violet-500/10 to-purple-500/5 border-violet-300',
  ...
  },
  {
    ...
    accent: 'from-rose-500/10 to-pink-500/5 border-rose-300',
  ...
  },
  {
    ...
    accent: 'from-amber-500/10 to-orange-500/5 border-amber-300',
    ...
    >
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-bold text-slate-900 tracking-tight">WorkTracker</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/login"
            className="hidden sm:inline-flex px-4 py-1.5 text-sm text-slate-700 hover:text-slate-900 transition-colors"
          >
            Sign in
          </Link>
          <Link
  ...
  return (
    <section className="relative min-h-screen flex items-center pt-16 overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-24 flex flex-col items-center text-center">
        ...
        {/* Headline */}
        <h1 className="text-5xl sm:text-6xl lg:text-7xl font-extrabold text-slate-900 leading-[1.06] tracking-tight max-w-4xl mb-6">
          Track time.{' '}
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          ...
          <Link
            to="/login"
            className="inline-flex items-center gap-2 px-6 py-3 text-sm font-medium text-slate-700 rounded-xl border border-slate-300 hover:border-slate-400 hover:text-slate-900 transition-all hover:bg-slate-100/60"
          >
        ...
        <div className="mt-16 w-full max-w-4xl relative">
          <div className="absolute -inset-1 rounded-2xl bg-gradient-to-b from-indigo-500/10 to-transparent blur-xl opacity-60" />
          <div className="relative rounded-2xl border border-slate-300 bg-white/80 backdrop-blur overflow-hidden shadow-2xl">
            {/* Window chrome */}
            <div className="flex items-center gap-1.5 px-4 py-3 border-b border-slate-200 bg-slate-50/60">
              <span className="w-2.5 h-2.5 rounded-full bg-rose-500/70" />
              <span className="w-2.5 h-2.5 rounded-full bg-amber-500/70" />
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-500/70" />
              <div className="ml-4 flex-1 h-5 rounded-md bg-slate-200 max-w-xs" />
            </div>
            {/* Mock dashboard content */}
            <div className="p-5 grid grid-cols-4 gap-3 mb-3">
              {[
                { label: 'Active Projects', val: '8', color: 'text-indigo-600' },
                { label: 'This week', val: '34.5h', color: 'text-emerald-600' },
                { label: 'Total hours', val: '412h', color: 'text-sky-600' },
                { label: 'Timecards', val: '76', color: 'text-violet-600' },
              ].map((s) => (
                <div key={s.label} className="rounded-xl border border-slate-200 bg-slate-100/50 p-4">
            ...
            <div className="px-5 pb-5 grid grid-cols-5 gap-3">
              <div className="col-span-3 rounded-xl border border-slate-200 bg-slate-100/30 h-36 p-4">
                ...
                {['Q4 Engineering Sprint', 'Frontend redesign', 'API v2 docs'].map((n, i) => (
                  <div key={n} className="flex items-center justify-between py-1.5 border-b border-slate-300/60 last:border-0">
                    <span className="text-xs text-slate-600">{n}</span>
                    <span className="text-xs font-semibold text-slate-700">{[8, 6.5, 4][i]}h</span>
                  </div>
                ))}
              </div>
              <div className="col-span-2 rounded-xl border border-slate-200 bg-slate-100/30 h-36 p-4">
...
function StatsSection() {
  ...
  return (
    <section ref={ref} className="border-y border-slate-200 bg-slate-100/40">
  ...
  return (
    <section id="features" className="py-28 max-w-6xl mx-auto px-6">
      <div className="text-center mb-16">
        <p className="text-xs font-semibold text-indigo-600 uppercase tracking-widest mb-3">Everything you need</p>
        <h2 className="text-4xl font-extrabold text-slate-900 tracking-tight">Built for how teams actually work</h2>
        <p className="text-slate-600 mt-4 max-w-lg mx-auto text-sm leading-relaxed">
      ...
      >
        {FEATURES.map((f, i) => (
          ...
          >
            <div className={`w-10 h-10 rounded-xl bg-slate-200 flex items-center justify-center mb-4 ${f.iconColor}`}>
...
function HowItWorksSection() {
  const { ref, visible } = useInView();
  return (
    <section id="how-it-works" className="py-28 border-t border-slate-200">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          <p className="text-xs font-semibold text-violet-600 uppercase tracking-widest mb-3">Simple by design</p>
        ...
        <div ref={ref} className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          {/* Connector line */}
          <div className="hidden md:block absolute top-8 left-[calc(16.67%)] right-[calc(16.67%)] h-px bg-gradient-to-r from-transparent via-slate-300 to-transparent" />
          ...
          {HOW_IT_WORKS.map((item, i) => (
            ...
            >
              <div className="w-16 h-16 rounded-2xl bg-slate-100 border border-slate-300 flex items-center justify-center mb-5 relative z-10">
                <span className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-br from-indigo-600 to-violet-600">
                  {item.step}
                </span>
              </div>
              <h3 className="text-sm font-semibold text-slate-900 mb-2">{item.title}</h3>
              <p className="text-xs text-slate-600 leading-relaxed max-w-[200px]">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section className="py-24 border-t border-slate-200">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-14">
          <p className="text-xs font-semibold text-sky-600 uppercase tracking-widest mb-3">Security first</p>
          <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">Built to be trusted</h2>
        </div>
        <div ref={ref} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {items.map((item, i) => (
            <div
              key={item.label}
              className="rounded-2xl border border-slate-200 bg-slate-100/40 p-6 hover:border-slate-300 transition-all duration-500 hover:-translate-y-1"
              style={{ transitionDelay: `${i * 70}ms`, opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(16px)' }}
            >
              <div className="w-9 h-9 rounded-xl bg-sky-600/10 flex items-center justify-center text-sky-600 mb-4">
    ...
    <section className="py-28 border-t border-slate-800">
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <p className="relative text-xs font-semibold text-indigo-600 uppercase tracking-widest mb-4">Get started today</p>
          <h2 className="relative text-4xl sm:text-5xl font-extrabold text-slate-900 tracking-tight mb-4 max-w-xl mx-auto">
            Your team's time is worth tracking right.
          </h2>
          <p className="relative text-slate-600 text-sm max-w-md mx-auto mb-10 leading-relaxed">
          ...
          <div className="relative flex flex-col sm:flex-row items-center justify-center gap-3">
            ...
            <Link
              to="/login"
              className="inline-flex items-center gap-2 px-7 py-3.5 text-sm font-medium text-slate-700 rounded-xl border border-slate-300 hover:border-slate-400 hover:bg-slate-100/60 hover:text-slate-900 transition-all"
            >
              Sign in
            </Link>
          </div>
    ...
    <footer className="border-t border-slate-200 bg-white">
      <div className="max-w-6xl mx-auto px-6 py-10 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-bold text-slate-700">WorkTracker</span>
        </div>
        <p className="text-xs text-slate-600">
          Built with FastAPI + React. MIT licensed.
        </p>
        <div className="flex items-center gap-5">
          <Link to="/login" className="text-xs text-slate-600 hover:text-slate-900 transition-colors">Sign in</Link>
          <Link to="/register" className="text-xs text-slate-600 hover:text-slate-900 transition-colors">Register</Link>
        </div>
      </div>
    </footer>
  );
