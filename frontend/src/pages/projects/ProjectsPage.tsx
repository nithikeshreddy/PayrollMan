    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-0.5">{projects?.length ?? 0} total</p>
        </div>
        {canCreateProjects && (
      ...
      {canManageAssignments && (
        <div className="flex gap-1 mb-5 bg-slate-100 rounded-lg p-1 border border-slate-200 w-fit">
          <button
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-600">
                        Wants to join <span className="text-indigo-600 font-medium">{assignment.project_name || `Project #${assignment.project_id}`}</span>
                        {assignment.project_code && (
                          <span className="ml-1 text-slate-500">({assignment.project_code})</span>
                        )}
                      </p>
                      {assignment.notes && (
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-6">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 rounded-lg border border-slate-300 bg-white text-sm text-slate-900 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500/50 transition-all hover:border-slate-400"
              />
            </div>
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value)}
              className="rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all cursor-pointer min-w-[160px]"
            >
              ...
              <button
                ...
                className={`px-3 py-1.5 text-xs font-medium rounded-full border transition-all ${
                  projectFilter === key
                    ...
                    : 'bg-white text-slate-700 border-slate-300 hover:text-slate-900 hover:border-slate-400'
                }`}
              >
                {label}
              </button>
...
function ProjectCard({
  ...
  return (
    <Card className="group hover:border-slate-300 transition-all duration-200">
      <div className="p-5 flex flex-col h-full">
        <div className="flex items-start justify-between mb-3">
          <div className="flex flex-col min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-mono text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md border border-indigo-200">
                {project.code}
              </span>
          ...
          <div className="relative ml-2">
            ...
            {menuOpen && (
              <div className="absolute right-0 top-8 z-10 min-w-[140px] rounded-xl border border-slate-200 bg-white shadow-xl py-1">
                <Link
                  to={`/projects/${project.id}`}
                  className="flex items-center gap-2.5 px-4 py-2 text-xs text-slate-700 hover:bg-slate-100 hover:text-slate-900 transition-colors"
        ...
        <div className="space-y-1.5 mb-3">
          <div className="flex items-center gap-2 text-xs text-slate-600">
            ...
            <span className="text-slate-900 font-medium">{project.creator_name ?? 'Unknown'}</span>
            {isCreator && <span className="text-indigo-600 text-[10px]">(you)</span>}
          </div>
          {(project.supervisor_name || project.supervisor_id) && (
            <div className="flex items-center gap-2 text-xs text-slate-600">
              <Shield className="w-3 h-3 text-slate-500" />
              <span className="text-slate-600">Supervisor</span>
              <span className="text-slate-900 font-medium">{project.supervisor_name ?? 'Unknown'}</span>
              {isSupervisor && <span className="text-indigo-600 text-[10px]">(you)</span>}
            </div>
          )}
          <div className="flex items-center gap-2 text-xs text-slate-600">
            <Users className="w-3 h-3 text-slate-500" />
            <span className="text-slate-600">Members</span>
            <span className="text-slate-900 font-medium">{project.assigned_users_count ?? 0}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-slate-200">
          <div className="flex flex-col gap-0.5">
            ...
            <span className="text-[10px] text-slate-600">{formatDate(project.created_at)}</span>
          </div>
...
function CreateProjectForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
      <div className="mb-1 flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-slate-600">Setup Flow</p>
          <p className="mt-1 text-sm text-slate-900">
            {step === 0 ? 'Create project' : step === 1 ? 'Attach category and grouped codes' : 'Define starter rules'}
          </p>
        </div>
        <div className="text-xs text-slate-600">Step {step + 1} of 3</div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <label className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
              ...
              />
              <div>
                <p className="text-sm font-medium text-slate-900">Require approval to join</p>
      ...
      {step === 1 && (
        <div className="space-y-4">
          <label className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            ...
            />
            <div>
              <p className="text-sm font-medium text-slate-900">Create tracking category after project creation</p>
              <p className="mt-1 text-xs text-slate-600">
            ...
            <>
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-slate-900">Tracking codes</p>
                    <p className="text-xs text-slate-600">These will be grouped automatically under the category above.</p>
                  </div>
                ...
                <div className="space-y-3">
                  {trackingCodes.map((code, index) => (
                    <div key={code.id} className="rounded-lg border border-slate-200 bg-white p-3">
                      <div className="mb-3 flex items-center justify-between">
                        <p className="text-xs font-semibold uppercase tracking-wide text-slate-600">Code {index + 1}</p>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="col-span-2 rounded-lg border border-slate-200 bg-white p-3">
                          <div className="mb-3 flex items-center justify-between">
                            <div>
                              <p className="text-xs font-semibold uppercase tracking-wide text-slate-600">Additional fields</p>
                              <p className="text-[11px] text-slate-600">Add any extra metadata you want to attach to this code.</p>
                            </div>
                          ...
                          {(trackingCodeExtras[code.id] ?? []).length === 0 ? (
                            <p className="text-xs text-slate-600">No extra fields yet.</p>
                          ) : (
      ...
      {step === 2 && (
        <div className="space-y-4">
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div className="mb-3 flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-slate-900">Starter rules</p>
                <p className="text-xs text-slate-600">Optional defaults and automation the manager can refine later.</p>
              </div>
            ...
            {trackingRules.length === 0 ? (
              <p className="text-sm text-slate-600">No rules yet. You can still create the project and add rules later.</p>
            ) : (
              <div className="space-y-3">
                {trackingRules.map((rule, index) => (
                  <div key={rule.id} className="rounded-lg border border-slate-200 bg-white p-3">
                    <div className="mb-3 flex items-center justify-between">
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-600">Rule {index + 1}</p>
                      <button
                        type="button"
                        className="text-xs text-rose-400"
      ...
      {errors.root && (
        <div className="rounded-lg border border-rose-300 bg-rose-50 px-4 py-3">
          <p className="text-xs text-rose-600">{errors.root.message}</p>
        </div>
      )}
