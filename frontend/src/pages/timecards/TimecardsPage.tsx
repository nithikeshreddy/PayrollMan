...
function getPunchInputClass(kind: 'in' | 'out', hasValue: boolean): string {
  const base =
    'h-7 w-full min-w-0 cursor-text rounded-md border px-1.5 text-center text-[11px] font-semibold tabular-nums outline-none transition-colors';

  if (kind === 'in') {
    return `${base} ${
      hasValue
        ? 'border-emerald-400/20 bg-emerald-500/10 text-emerald-100'
        : 'border-transparent bg-transparent text-slate-500 hover:text-emerald-200'
    } hover:border-emerald-400/30 hover:bg-emerald-500/10 focus:border-emerald-300/70 focus:bg-slate-900/90 focus:text-emerald-100 focus:ring-1 focus:ring-emerald-400/25`;
  }

  return `${base} ${
    hasValue
      ? 'border-rose-400/20 bg-rose-500/10 text-rose-100'
      : 'border-transparent bg-transparent text-slate-500 hover:text-rose-200'
  } hover:border-rose-400/30 hover:bg-rose-500/10 focus:border-rose-300/70 focus:bg-slate-900/90 focus:text-rose-100 focus:ring-1 focus:ring-rose-400/25`;
}

function getAllocationInputClass(canEdit: boolean, hasValue: boolean): string {
  const base =
    'h-7 w-full min-w-0 rounded-md border px-1.5 text-center text-xs font-semibold tabular-nums outline-none transition-colors';

  if (!canEdit) {
    return `${base} cursor-default border-transparent bg-transparent text-slate-500 placeholder-slate-600`;
  }

  return `${base} cursor-text ${
    hasValue
      ? 'border-slate-600/30 bg-slate-800/30 text-slate-50'
      : 'border-transparent bg-transparent text-slate-100 placeholder-slate-500'
  } hover:border-slate-500/50 hover:bg-slate-800/50 focus:border-indigo-300/70 focus:bg-slate-900/90 focus:ring-1 focus:ring-indigo-400/30`;
}
...
export function TimecardsPage() {
  ...
  const showPunchInputs =
    viewedUserId !== null &&
    !isViewingOtherUser &&
    currentWorkflowStatus !== 'approved' &&
    (currentWorkflowStatus === 'draft' || manualEditMode);
  ...
  const workflowConfig: Record<TimecardSubmission['status'], { label: string; cls: string }> = {
    draft: { label: 'Draft', cls: 'text-slate-700 bg-slate-100 border-slate-300' },
    submitted: { label: 'Submitted', cls: 'text-amber-700 bg-amber-50 border-amber-200' },
    on_hold: { label: 'On Hold', cls: 'text-rose-700 bg-rose-50 border-rose-200' },
    ...
    <div className="w-full">
      ...
      <div className="mb-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="min-w-0">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-slate-900">Weekly Timecard</h1>
          </div>
          <div className="mt-1.5 flex flex-wrap items-center gap-2">
            ...
            {isViewingOtherUser && (
              <span className="inline-flex rounded-full border border-sky-300 bg-sky-50 px-2.5 py-0.5 text-xs font-medium text-sky-700">
          ...
          <div className="mt-3 max-w-3xl space-y-2.5">
            ...
            {!isViewingOtherUser && (currentWorkflowStatus === 'submitted' || currentWorkflowStatus === 'on_hold') && (
              <div className="rounded-xl border border-sky-200 bg-sky-50 px-3 py-2.5 text-sm text-sky-700">
                {manualEditMode
        ...
        <div className="relative z-[120] flex flex-wrap items-start justify-end gap-2 xl:items-start">
          <div className="flex items-center gap-0.5 rounded-lg border border-slate-300 bg-white px-1">
            <button
              onClick={() => setWeekStart((d) => addDays(d, -7))}
              className="rounded p-1.5 text-slate-600 transition-colors hover:text-slate-900"
            >
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="px-2 text-xs font-medium whitespace-nowrap text-slate-700">
          ...
          {!isViewingOtherUser &&
            viewedUserId !== null &&
            currentWorkflowStatus !== 'approved' &&
            (manualEditMode || currentWorkflowStatus === 'submitted' || currentWorkflowStatus === 'on_hold') && (
              <Button
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start">
          {/* ── Main grid ── */}
          <div className="min-w-0 overflow-x-auto overflow-y-auto rounded-xl border border-slate-700/80 bg-slate-950 max-h-[calc(100vh-180px)] xl:flex-1 shadow-xl shadow-slate-900/15">
            <table className="min-w-[840px] w-full table-fixed border-collapse text-sm">
              <colgroup>
                <col style={{ width: '176px' }} />
                {weekDays.map((d) => (
                  <col key={d.toISOString()} style={{ width: '94px' }} />
                ))}
              </colgroup>
              ...
              <thead>
                <tr className="border-b border-slate-600/80">
                  <th className="sticky top-0 left-0 z-40 w-[176px] min-w-[176px] bg-slate-800 px-3 py-3" />
                  {weekDays.map((day) => {
                    const dateStr = format(day, 'yyyy-MM-dd');
                    ...
                    return (
                      <th
                        key={day.toISOString()}
                        className={`sticky top-0 z-30 border-l border-slate-700/70 px-3 py-3 text-center ${
                          isToday ? 'bg-indigo-950/70' : 'bg-slate-800'
                        }`}
                      >
                        <p
                        ...
                        <p
                          className={`text-base font-bold mt-0.5 leading-none ${
                            isToday ? 'text-indigo-200' : 'text-slate-100'
                          }`}
                        >
                          {format(day, 'd')}
                        </p>
                ...
                <tr className="border-y border-slate-600/80">
                  <th className="sticky top-[84px] left-0 z-40 w-[176px] min-w-[176px] whitespace-nowrap border-r border-slate-700/70 bg-slate-800 px-3 py-2.5">
                    <div className="flex items-center gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-indigo-300" />
                      <span className="text-xs font-bold text-slate-50 uppercase tracking-wide">
                        Total Hours
                      </span>
                    </div>
                  </th>
                  {weekDays.map((day) => {
                    ...
                    return (
                      <th
                        ...
                        className={`sticky top-[84px] z-20 border-l border-slate-700/70 px-2 py-2.5 text-center ${
                          isToday ? 'bg-indigo-950/60' : 'bg-slate-800/80'
                        }`}
                      >
                        <span
                          className={`text-sm font-bold tabular-nums ${
                            mins > 0 ? 'text-slate-50' : 'text-slate-500'
                          }`}
                        >
              ...
              <tbody>
                ...
                {Array.from({ length: maxPairs }, (_, i) => (
                  <Fragment key={`punch-pair-${i}`}>
                    ...
                    >
                      <td className="sticky left-0 z-10 w-[176px] min-w-[176px] whitespace-nowrap border-r border-slate-700/70 bg-slate-900 px-3 py-2">
                        <div className="flex items-center gap-2">
                          ...
                          {showPunchInputs && i >= basePunchPairCount && (
                            <span className="rounded-full bg-sky-500/10 px-1.5 py-0.5 text-[10px] font-medium text-sky-300">new row</span>
                          )}
                        </div>
                      </td>
                      {weekDays.map((day) => {
                        ...
                        return (
                          <td
                            key={day.toISOString()}
                            className={`h-10 border-l border-slate-800/80 px-1.5 py-1.5 text-center align-middle transition-colors ${
                              isToday ? 'bg-indigo-500/5' : ''
                            } ${showPunchInputs ? 'hover:bg-slate-800/40' : ''}`}
                          >
                            {showPunchInputs ? (
                              <input
                                type="time"
                                ...
                                className={getPunchInputClass('in', (punchDrafts[`${dateStr}_${i}`]?.punchIn ?? '') !== '')}
                              />
                            ) : entry ? (
                    ...
                    {/* Punch Out row */}
                    <tr key={`out-${i}`} className="border-t border-slate-800/80">
                      <td className="sticky left-0 z-10 w-[176px] min-w-[176px] whitespace-nowrap border-r border-slate-700/70 bg-slate-900 px-3 py-2">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {showPunchInputs ? (
                              <input
                                ...
                                onBlur={(e) => handlePunchBlur(e, dateStr, i)}
                                data-punch-row={`${dateStr}_${i}`}
                                className={getPunchInputClass('out', (punchDrafts[`${dateStr}_${i}`]?.punchOut ?? '') !== '')}
                              />
                            ) : entry?.punch_out ? (
                              ...
                              <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[10px] font-semibold text-emerald-300 animate-pulse">
                                Active
                              </span>
                            ) : (
                              <span className="text-slate-500 text-xs">—</span>
                ...
                <tr className="border-t border-slate-700/70">
                  <td colSpan={8} className="py-1.5 bg-slate-950" />
                </tr>
                <tr className="border-y border-slate-600/80">
                  <td
                    colSpan={8}
                    className="bg-slate-800/80 px-3 py-2.5"
                  >
                    <div className="flex items-center gap-2">
                ...
                {allocationBuckets.length === 0 ? (
                  ...
                  groupedBuckets.flatMap((group, groupIndex) => {
                    ...
                    const rows = [
                      <tr key={`group-${group.key}`} className={`bg-slate-800/30 border-t ${groupIndex === 0 ? 'border-slate-600/80' : 'border-slate-700/70'}`}>
                        <td className="sticky left-0 z-10 w-[176px] min-w-[176px] border-r border-slate-700/70 bg-slate-800 px-3 py-2.5">
                          <button
                            type="button"
                            onClick={() => toggleGroup(group.key)}
                            className="flex w-full items-start justify-between gap-2 rounded-md text-left transition-colors hover:bg-slate-700/40"
                          >
                            <div className="min-w-0">
                              <p className="max-w-[136px] truncate text-[11px] font-bold uppercase tracking-wide text-slate-50">{group.title}</p>
                              <p className="mt-1 max-w-[136px] truncate text-[10px] leading-4 text-slate-400">{group.description}</p>
                            </div>
                            {isCollapsed ? (
                              <ChevronDown className="mt-0.5 h-4 w-4 flex-shrink-0 text-slate-300" />
                            ) : (
                              <ChevronUp className="mt-0.5 h-4 w-4 flex-shrink-0 text-slate-300" />
                            )}
                          </button>
                        </td>
                        {weekDays.map((day) => {
                          ...
                          return (
                            <td
                              ...
                              className={`h-10 border-l border-slate-700/60 px-2 py-2 text-center align-middle ${
                                isToday ? 'bg-indigo-500/10' : 'bg-slate-800/30'
                              }`}
                            >
                              <span className={`text-xs font-bold tabular-nums ${total > 0 ? 'text-slate-50' : 'text-slate-500'}`}>
                                {total > 0 ? `${total}h` : '—'}
                              </span>
                            </td>
                          );
                    ...
                    if (!isCollapsed) {
                      rows.push(
                        ...group.buckets.map((bucket, idx) => (
                          <tr
                            key={bucket.key}
                            className={`border-t border-slate-800/80 ${idx % 2 !== 0 ? 'bg-slate-800/10' : 'bg-slate-900/5'}`}
                          >
                            <td className="sticky left-0 z-10 w-[176px] min-w-[176px] border-r border-slate-700/70 bg-slate-900 px-3 py-2">
                              <p
                                className="max-w-[140px] truncate text-xs font-medium text-slate-100"
                                title={bucket.label}
                              >
                                {bucket.label}
                              </p>
                              <p className="mt-0.5 max-w-[140px] truncate text-[10px] text-slate-400">{bucket.subtitle ?? (bucket.source === 'category' ? 'Configured tracking code' : 'Legacy entry')}</p>
                            </td>
                            {weekDays.map((day) => {
                              const dateStr = format(day, 'yyyy-MM-dd');
                              const key = `${bucket.key}_${dateStr}`;
                              ...
                              return (
                                <td
                                  ...
                                  className={`h-10 border-l border-slate-800/80 px-1.5 py-1.5 text-center align-middle transition-colors ${
                                    canEditViewedWeek
                                      ? (isToday ? 'bg-indigo-500/5 hover:bg-indigo-500/10' : 'hover:bg-slate-800/40')
                                      : (isToday ? 'bg-indigo-950/15' : 'bg-slate-950/5')
                                  }`}
                                >
                                  <input
                                    type="number"
                                    ...
                                    className={getAllocationInputClass(canEditViewedWeek, (localHours[key] ?? '') !== '')}
                                  />
                                </td>
                              );
                            })}
                ...
                <tr className="border-t border-slate-600/80">
                  <td className="sticky left-0 z-10 w-[176px] min-w-[176px] whitespace-nowrap border-r border-slate-700/70 bg-slate-800 px-3 py-2.5">
                    <div className="flex items-center gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-indigo-300" />
                      <span className="text-xs font-bold text-slate-50 uppercase tracking-wide">
                        Total Hours
                  ...
                  {weekDays.map((day) => {
                    ...
                    return (
                      <td
                        key={day.toISOString()}
                        className={`h-10 border-l border-slate-700/60 px-2 py-2 text-center align-middle ${
                          isToday ? 'bg-indigo-500/10' : 'bg-slate-800/70'
                        }`}
                      >
                        <span
                          className={`text-sm font-bold tabular-nums ${
                            total > 0 ? 'text-slate-50' : 'text-slate-500'
                          }`}
                        >
                          {total > 0 ? `${total}h` : '—'}
                        </span>
          ...
          <div className="w-full flex-shrink-0 flex flex-col gap-3 xl:w-48">
            <div className="rounded-xl border border-slate-200 bg-white p-3 text-left">
              ...
              <p className="text-xs text-slate-700">Current status</p>
              <p className="mt-1 text-[11px] leading-5 text-slate-800 font-medium">
                {workflowConfig[currentWorkflowStatus].label}
              </p>
              {weekSubmission?.submitted_at && (
                <>
                  <p className="mt-3 text-xs text-slate-700">Submitted</p>
                  <p className="mt-1 text-[11px] leading-5 text-slate-800">{format(new Date(weekSubmission.submitted_at), 'MMM d, yyyy p')}</p>
                </>
              )}
              {weekSubmission?.approved_at && (
                <>
                  <p className="mt-3 text-xs text-slate-700">Approved</p>
                  <p className="mt-1 text-[11px] leading-5 text-slate-800">{format(new Date(weekSubmission.approved_at), 'MMM d, yyyy p')}</p>
                </>
              )}
              {(weekSubmission?.unresolved_issue_count ?? 0) > 0 && (
                <>
                  <p className="mt-3 text-xs text-slate-700">Open issues</p>
                  <p className="mt-1 text-[11px] leading-5 text-slate-800 font-medium">{weekSubmission?.unresolved_issue_count}</p>
                </>
              )}
            </div>
            <div className={`rounded-xl p-4 border ${weeklyLimit != null ? 'border-amber-200 bg-amber-50' : 'border-indigo-200 bg-indigo-50'} text-center`}>
              <p className={`text-[10px] uppercase tracking-widest font-semibold leading-relaxed mb-3 ${weeklyLimit != null ? 'text-amber-700' : 'text-indigo-700'}`}>
                {weeklyLimit != null ? 'Weekly Progress' : 'Total Hours'}<br />for Week
              </p>
              <p className={`text-3xl font-bold tabular-nums ${weeklyLimit != null ? 'text-amber-800' : 'text-indigo-800'}`}>
              ...
              {weeklyLimit != null ? (
                <div className="mt-3 space-y-2 text-left">
                  ...
                  <div className="h-2 overflow-hidden rounded-full bg-amber-200">
                    <div
                      className={`h-full rounded-full ${weeklyOverage > 0 ? 'bg-rose-500' : 'bg-amber-500'}`}
                      style={{ width: `${Math.min((weekTotalHours / weeklyLimit) * 100, 100)}%` }}
                    />
                  </div>
                  <p className={`text-[10px] ${weeklyOverage > 0 ? 'text-rose-700' : 'text-amber-700'}`}>
                    {weeklyOverage > 0
                  ...
                  {!!effectiveRule?.applied_rule_names?.length && (
                    <div className="rounded-lg border border-amber-200 bg-amber-100/50 px-2 py-2 text-[10px] text-amber-800">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-3 text-center">
              ...
              <p className="text-2xl font-bold text-slate-900">
                ...
                <span className="text-sm text-slate-600 font-normal">/7</span>
              </p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white p-3 text-left">
              <p className="text-[10px] text-slate-600 uppercase tracking-widest font-semibold mb-2">
              ...
              <p className="text-xs text-slate-700">Configured categories</p>
              <p className="mt-1 text-[11px] leading-5 text-slate-800 font-medium">
                {trackingCategories?.length ?? 0}
              </p>
              <p className="mt-3 text-xs text-slate-700">Active tracking codes</p>
              <p className="mt-1 text-[11px] leading-5 text-slate-800 font-medium">
