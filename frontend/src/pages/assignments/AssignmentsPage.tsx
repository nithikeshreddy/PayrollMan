      ...
      <div className="flex gap-1 mb-6 bg-slate-100 rounded-xl p-1 w-fit border border-slate-200">
        {STATUS_TABS.map((tab) => {
          ...
          return (
            ...
            >
              {tab} {count !== undefined && (
                <span className={`ml-1 text-[10px] ${activeTab === tab ? 'text-indigo-100' : 'text-slate-500'}`}>
                  {count}
                </span>
              )}
            </button>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {project?.code && (
                          <span className="text-[10px] font-mono text-indigo-700 bg-indigo-100 px-1.5 py-0.5 rounded border border-indigo-200">
                            {project.code}
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-xs text-slate-600">{userMap[assignment.user_id]?.email ?? ''}</span>
                        {assignment.role && (
                          <>
                            <span className="text-slate-400">·</span>
                            <span className="text-xs text-slate-600 capitalize">{assignment.role}</span>
                          </>
                        )}
                        <span className="text-slate-400">·</span>
