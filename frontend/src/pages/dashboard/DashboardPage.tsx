    ...
    <div className="max-w-6xl mx-auto space-y-6">
        ...
        >
          {activePunch ? (
            <>
              ...
              <span className="text-sm font-mono font-bold text-emerald-600 tabular-nums w-[72px]">
                {elapsed}
              </span>
            ...
            <>
              ...
              <span className="text-xs text-slate-700">Not clocked in</span>
              <div className="flex items-center gap-1 ml-1">
                ...
                <button
                  ...
                  className="p-1.5 rounded-md text-slate-700 hover:text-slate-900 hover:bg-slate-200/50 transition-colors"
                  title="Add note"
                >
      ...
      {showNotes && !activePunch && (
        <input
          ...
          className="w-full sm:max-w-sm rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all -mt-3"
        />
      )}

      {/* ─── Row 2: Quick Stats ─── */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <MiniStat label="Today" value={minutesToHours(todayMinutes)} accent="text-emerald-600" icon={<Clock className="w-3.5 h-3.5" />} />
        <MiniStat label="This Week" value={minutesToHours(thisWeekPunchMinutes)} accent="text-indigo-600" icon={<Calendar className="w-3.5 h-3.5" />} />
        <MiniStat label="Projects" value={activeProjects} accent="text-violet-600" icon={<FolderKanban className="w-3.5 h-3.5" />} />
        <MiniStat label="Timecards" value={`${totalHours.toFixed(1)}h`} accent="text-sky-600" icon={<TrendingUp className="w-3.5 h-3.5" />} />
      </div>
        ...
        <Card className="lg:col-span-3">
          <CardHeader className="py-3.5 px-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xs font-semibold text-slate-700 uppercase tracking-wider">Recent Timecards</h2>
            </div>
            <Link
              to="/timecards"
                ...
                {recentTimecards.map((tc) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 ml-3 shrink-0">
                      {tc.project_code && (
                        <span className="text-[10px] text-indigo-700 bg-indigo-100 px-1.5 py-0.5 rounded border border-indigo-200 font-mono">
                          {tc.project_code}
                        </span>
                      )}
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="py-3.5 px-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xs font-semibold text-slate-700 uppercase tracking-wider">Projects</h2>
            </div>
            <Link
              to="/projects"
              className="text-[11px] text-indigo-600 hover:text-indigo-700 flex items-center gap-0.5"
            ...
            {recentProjects.length === 0 ? (
              ...
              <div className="divide-y divide-slate-200">
                {recentProjects.map((project) => (
                  ...
                  >
                    <div className="flex flex-col min-w-0">
                      <span className="text-sm text-slate-900 truncate font-medium">{project.name}</span>
        ...
        <Card>
          <CardHeader className="py-3.5 px-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xs font-semibold text-slate-700 uppercase tracking-wider">Attendance</h2>
            ...
            {weekGroups.length > 1 && (
              <button
                ...
                className="text-[11px] text-indigo-600 hover:text-indigo-700 flex items-center gap-0.5 transition-colors"
              >
              ...
              {visibleWeeks.map((wg) => (
                <div key={wg.key}>
                  ...
                  <div className="flex items-center justify-between px-5 py-2 bg-slate-100">
                    <div className="flex items-center gap-2">
                      <span className="text-[11px] font-medium text-slate-700">{wg.label}</span>
                      {wg.isCurrent && (
                        <span className="text-[9px] font-semibold text-indigo-700 bg-indigo-100 px-1.5 py-px rounded-full border border-indigo-200">
                          Current
                        </span>
                      )}
                    </div>
                  ...
                  {wg.entries.map((entry) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-[10px] uppercase tracking-wide text-slate-700 font-semibold w-7 shrink-0">
                          {dayName}
                        </span>
                        <div className="flex items-center gap-1 text-xs">
                          <span className="text-emerald-700">{formatTime(entry.punch_in)}</span>
                        ...
                        <button
                          ...
                          className="p-0.5 rounded text-slate-600 hover:text-rose-600 opacity-0 group-hover:opacity-100 transition-all"
                          title="Delete"
                        >
...
function MiniStat({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 flex items-center gap-3 hover:border-slate-300 transition-colors">
      <div className={`flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 ${accent}`}>
        {icon}
      </div>
      <div>
